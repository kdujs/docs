...
In large applications, we may need to divide the app into smaller chunks and only load a component from the server when it's needed. To make that possible, Kdu has a [`defineAsyncComponent`](/api/general.html#defineasynccomponent) function:
...
export default {
  components: {
    AdminPage: defineAsyncComponent(() =>
      import('./components/AdminPageComponent.kdu')
    )
  }
}
</script>
