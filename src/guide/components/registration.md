...
A Kdu component needs to be "registered" so that Kdu knows where to locate its implementation when it is encountered in a template. There are two ways to register components: global and local.

## Global Registration

We can make components available globally in the current [Kdu application](/guide/essentials/application.html) using the `app.component()` method:
...
If using SFCs, you will be registering the imported `.kdu` files:

```js
import MyComponent from './App.kdu'
...
2. `<PascalCase />` makes it more obvious that this is a Kdu component instead of a native HTML element in templates. It also differentiates Kdu components from custom elements (web components).
...
Luckily, Kdu supports resolving kebab-case tags to components registered using PascalCase. This means a component registered as `MyComponent` can be referenced in the template via both `<MyComponent>` and `<my-component>`. This allows us to use the same JavaScript component registration code regardless of template source.
