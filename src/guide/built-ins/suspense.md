...
In addition, these components are often used in conjunction with the `<RouterView>` component from [Kdu Router](https://kdujs-router.web.app/).
...
```kdu-html
<RouterView k-slot="{ Component }">
  <template k-if="Component">
...
Kdu Router has built-in support for [lazily loading components](https://kdujs-router.web.app/guide/advanced/lazy-loading.html) using dynamic imports. These are distinct from async components and currently they will not trigger `<Suspense>`. However, they can still have async components as descendants and those can trigger `<Suspense>` in the usual way.
