...
Plugins are self-contained code that usually add app-level functionality to Kdu. This is how we install a plugin:

```js
import { createApp } from 'kfu'
...
4. A library that needs to perform some combination of the above (e.g. [kdu-router](https://kdujs-router.web.app)).

## Writing a Plugin

In order to better understand how to create your own Kdu.js plugins, we will create a very simplified version of a plugin that displays `i18n` (short for [Internationalization](https://en.wikipedia.org/wiki/Internationalization_and_localization)) strings.
...
See also: [Augmenting Global Properties](/guide/typescript/options-api.html#augmenting-global-properties) <sup class="kt-badge ts" />
