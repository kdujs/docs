...
Typically this is done on the top of `App.kdu` as it will be the first focusable element on all your pages:
...
Once a user changes route, bring focus back to the skip link. This can be achieved by calling focus on the skip link's template ref (assuming usage of `kdu-router`):
...
<form action="/dataCollectionLocation" method="post" autocomplete="on">
  <div k-for="item in formItems" :key="item.id" class="form-item">
...
<input type="text" name="name" id="name" k-model="name" />
```
...
One of the issues with placeholders is that they don't meet the [color contrast criteria](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html) by default; fixing the color contrast makes the placeholder look like pre-populated data in the input fields.
...
<form role="search">
  ...
  <input type="text" name="search" id="search" k-model="search" />
...
<form role="search">
  ...
  <input type="text" name="searchIcon" id="searchIcon" k-model="searchIcon" />
  <button type="submit">
