...
:::tip
While Kdu does support TypeScript usage with Options API, it is recommended to use Kdu with TypeScript via Composition API as it offers simpler, more efficient and more robust type inference.
:::

## Typing Component Props

Type inference for props in Options API requires wrapping the component with `defineComponent()`. With it, Kdu is able to infer the types for the props based on the `props` option, taking additional options such as `required: true` and `default` into account:
...
Some plugins, for example `kdu-router`, provide support for custom component options such as `beforeRouteEnter`:
...
Now the `beforeRouteEnter` option will be properly typed. Note this is just an example - well-typed libraries like `kdu-router` should automatically perform these augmentations in their own type definitions.
