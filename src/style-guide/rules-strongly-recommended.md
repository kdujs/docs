...
**Base components (a.k.a. presentational, dumb, or pure components) that apply app-specific styling and conventions should all begin with a specific prefix, such as `Base`, `App`, or `K`.**
...
- Since component names should always be multi-word, this convention prevents you from having to choose an arbitrary prefix for simple component wrappers (e.g. `MyButton`, `KduButton`).
...
- Name conflicts (e.g. multiple `ButtonDelete.kdu` components) make it more difficult to quickly navigate to a specific component in a code editor.
...
Unfortunately, HTML doesn't allow custom elements to be self-closing - only [official "void" elements](https://www.w3.org/TR/html/syntax.html#void-elements). That's why the strategy is only possible when Kdu's template compiler can reach the template before the DOM, then serve the DOM spec-compliant HTML.
...
- If you use any non-Kdu custom elements in your templates, such as a web component, PascalCase ensures that your Kdu components remain distinctly visible.
...
**Directive shorthands (`:` for `k-bind:`, `@` for `k-on:` and `#` for `k-slot`) should be used always or never.**
