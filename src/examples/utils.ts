import { onBeforeUnmount } from 'kdu'
...
function injectCreateApp(src: string): string {
  const importKduRE = /import {(.*?)} from 'kdu'/
  if (importKduRE.test(src)) {
    src = src.replace(importKduRE, `import { createApp,$1} from 'kdu'`)
  } else {
    const newline = src.startsWith(`import`) ? `\n` : `\n\n`
    src = `import { createApp } from 'kdu'${newline}${src}`
  }
  return src.replace(
    /export default ({[^]*\n})/,
