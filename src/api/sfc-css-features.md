...
```kdu
<style>
.example[data-k-f3f3eg9] {
  color: red;
}
</style>

<template>
  <div class="example" data-k-f3f3eg9>hi</div>
</template>
```
...
:::tip
DOM content created with `k-html` are not affected by scoped styles, but you can still style them using deep selectors.
:::
...
SFC `<style>` tags support linking CSS values to dynamic component state using the `k-bind` CSS function:
