...
An SFC can implicitly refer to itself via its filename. E.g. a file named `FooBar.kdu` can refer to itself as `<FooBar/>` in its template.
...
Globally registered custom directives just work as normal. Local custom directives don't need to be explicitly registered with `<script setup>`, but they must follow the naming scheme `kNameOfDirective`:
...
<template>
  <h1 k-my-directive>This is a Heading</h1>
</template>
```
...
```kdu
<script setup>
import { myDirective as kMyDirective } from './MyDirective.js'
</script>
```
...
```kdu
<script setup>
import { useSlots, useAttrs } from 'kdu'
...
## TypeScript-only Features <sup class="kt-badge ts" />
