...
Exposes the current version of Kdu.
...
- **Details**
  ...
  When you mutate reactive state in Kdu, the resulting DOM updates are not applied synchronously. Instead, Kdu buffers them until the "next tick" to ensure that each component updates only once no matter how many state changes you have made.
...
A type helper for defining a Kdu component with type inference.
...
- **Details**
  ...
  Note this is not necessary if you are using Wite, because Rollup (the underlying production bundler used by Wite) is smart enough to determine that `defineComponent()` is in fact side-effect-free without the need for manual annotations.

- **See also:** [Guide - Using Kdu with TypeScript](/guide/typescript/overview.html#general-usage-notes)
...
- **See also:**
  ...
  - [Guide - Building Custom Elements with Kdu](/guide/extras/web-components.html#building-custom-elements-with-kdu)
