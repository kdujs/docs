...
- **Details**
  ...
  This config option is only respected when using the full build (i.e. the standalone `kdu.js` that can compile templates in the browser). It supports the same options as the app-level [app.config.compilerOptions](/api/application.html#app-config-compileroptions), and has higher priority for the current component.
