...
- **Details**
  ...
  When using TypeScript, the key can be a symbol casted as `InjectionKey` - a Kdu provided utility type that extends `Symbol`, which can be used to sync the value type between `provide()` and `inject()`.
...
- **Details**
  ...
  The first argument is the injection key. Kdu will walk up the parent chain to locate a provided value with a matching key. If multiple components in the parent chain provides the same key, the one closest to the injecting component will "shadow" those higher up the chain. If no value with matching key was found, `inject()` returns `undefined` unless a default value is provided.
  ...
  When using TypeScript, the key can be of type of `InjectionKey` - a Kdu-provided utility type that extends `Symbol`, which can be used to sync the value type between `provide()` and `inject()`.
