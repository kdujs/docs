...
- **Example**
  ...
  ```kdu-html
  <Transition>
    <div k-if="ok">toggled content</div>
  </Transition>
  ```
  ...
  <Transition @after-enter="onTransitionComplete">
    <div k-show="ok">toggled content</div>
  </Transition>
  ```
...
- **Example**
  ...
  <TransitionGroup tag="ul" name="slide">
    <li k-for="item in items" :key="item.id">
      {{ item.text }}
    </li>
  </TransitionGroup>
  ```
...
- **Example**
  ...
  When used with `k-if` / `k-else` branches, there must be only one component rendered at a time:

  ```kdu-html
  <KeepAlive>
    <comp-a k-if="a > 1"></comp-a>
    <comp-b k-else></comp-b>
  </KeepAlive>
  ```
...
## `<Suspense>` <sup class="kt-badge experimental" />
